    ...
    <div className="relative inline-flex items-center justify-center">
      <svg width={size} height={size}>
        ...
        {segments.map((segment, index) => {
          ...
          const adjustedSegmentDegrees = Math.max(segmentDegrees - (2 * compensation) - extraReduction, 1);
      ...
      {(centerLabel || centerValue) && (
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          {centerValue && (
            <p className="font-['Crimson_Pro:ExtraLight',sans-serif] font-extralight text-[#555555] tracking-[-0.48px] text-[24px]">
              {centerValue}
            </p>
          )}
          {centerLabel && (
