...
| ~~ISS-002~~ | ~~No authentication layer~~ | Resolved — Cookie-session auth implemented with express-session + connect-pg-simple, bcrypt password hashing, 12h rolling sessions, `requireAuth` middleware. 4 demo users seeded. Ada-branded login page with dev quick-access persona picker. See Project Tasks #1–#2 (Login Page). | `server/routes/api.ts`, `server/index.ts` | 2026-03-21 |
...
| ISS-028 | Discover card backfill via seed.sql instead of migration | Task #9 backfilled existing discover cards with enriched `supporting_articles` data (url, summary) directly in `seed.sql`. In production, this should be a dedicated SQL migration script to ensure idempotent, reviewable data changes. | `server/db/seed.sql` | 2026-04-01 |
| ISS-029 | Article summary truncation not explicitly enforced | ArticleSourcesSheet displays article summaries with CSS 2-line clamp, but the backend does not explicitly truncate summaries to ~120 chars before storing in `supporting_articles` JSONB. Long summaries may affect storage size. | `server/services/discoverPipeline/synthesisWorker.ts`, `ArticleSourcesSheet.tsx` | 2026-04-01 |
...
| ISS-002 | No authentication layer | Cookie-session auth implemented with express-session + connect-pg-simple, bcrypt, 12h rolling sessions, requireAuth middleware, 4 demo users, Ada-branded login page (Project Tasks #1–#2) | 2026-04-01 |
