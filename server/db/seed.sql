...
-- that have card_id but are missing publisher/url, by pulling from the
-- referenced source card. Entries already having a publisher are left intact.
-- ============================================================
UPDATE discover_cards av
SET supporting_articles = (
  SELECT jsonb_agg(
    CASE
      WHEN (elem->>'publisher') IS NOT NULL AND (elem->>'publisher') != ''
      THEN elem
      WHEN (elem->>'card_id') IS NOT NULL
           AND src.supporting_articles IS NOT NULL
           AND jsonb_array_length(src.supporting_articles) > 0
      THEN elem
           || jsonb_build_object(
                'publisher', (src.supporting_articles->0)->>'publisher',
                'published_at', COALESCE((src.supporting_articles->0)->>'published_at', elem->>'published_at'),
...
WHERE av.card_type = 'ada_view'
  AND EXISTS (
    ...
    WHERE (e->>'card_id') IS NOT NULL
      AND ((e->>'publisher') IS NULL OR (e->>'publisher') = '')
  );
