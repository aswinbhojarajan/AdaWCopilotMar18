...
| **Structured Responses** | Built | Zod-validated AdaAnswer schema with headline, summary, citations, recommendations, actions, render hints. Frontend block components (ChatResponseRenderer, HoldingsTable, AllocationCard, DisclaimerTray, SourcesTray, FollowUpChips) render structured envelopes via BLOCK_REGISTRY discriminated union. `structured_intent` SSE event emitted before streaming. |
| **Disclaimer Footer** | Built | Disclosures removed from inline chat body. Backend emits `disclosures` SSE event. Persistent "Regulatory info" footer below chat input with CSS-transition bottom-sheet popup. Falls back to UAE regulatory defaults when no server disclosures available. |
| **Content Moderation** | Built | Pre-LLM input moderation + post-LLM output moderation via moderationService.ts (OpenAI omni-moderation-latest). Moderation events persisted. Bypass for deterministic Lane 0 requests. |
