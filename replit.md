...
- **17 Services**:
    ...
    - `modelRouter.ts`: Lane-based multi-model routing (Lane 1 fast, Lane 2 reasoning) with request scorecards, provider aliases, per-lane token/temperature budgets. Lane 0 type exists but is never routed to — all queries go through LLM.
