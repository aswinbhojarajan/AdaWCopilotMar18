...
- **Editorial Content Pipeline**: `editorialContentWorker.ts` generates `explainer` (whatsNew tab), `wealth_planning` (forYou tab), and `product_opportunity` (forYou tab) cards from a curated `editorial_content` corpus table (16 seed entries). LLM refreshes content with current market context. Runs on the 6-hour editorial timer alongside Ada View and Event Calendar. Per-type quotas: 2 explainers, 1 wealth_planning, 1 product_opportunity (checked via recent card count).
