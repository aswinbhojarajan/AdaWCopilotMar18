...
- **17 Services**:
    ...
    - `modelRouter.ts`: 2-lane multi-model routing (Lane 1 standard, Lane 2 reasoning) with request scorecards, provider aliases, per-lane token/temperature budgets. Both lanes include market_intel tools. All queries route through LLM.
...
- **Capability Registry** (`capabilityRegistry.ts`): Model capability registry mapping provider aliases (ada-fast, ada-reason, ada-fallback) to capabilities (streaming, tool_calling, json_mode, reasoning, etc.), cost tiers, and context limits. All three aliases support tool_calling. Also contains intent→lane→tool routing metadata (LaneConfig for 2 lanes, IntentRouteConfig) injected into classifier prompt via `getClassifierContext()`. Used by orchestrator for model selection, prompt builder for capability context, and verbose mode output.
- **Fast-Path Optimization**: Simple lookup intents (`balance_query`, `allocation_breakdown`, `goal_progress`) with `low` reasoning effort get `fast_path: true` in the route decision. Fast-path activation requires both `route.fast_path` AND `isFastPathPrefetchComplete()` — an intent-aware check that verifies all `FAST_PATH_REQUIRED_SOURCES` (e.g., `portfolio_api` for all 3 intents) have successful prefetch results. When `fastPathActive` is true: (a) max_tokens is 1024 (vs 2048/4096 standard), (b) tool definitions are omitted — all data pre-fetched and injected into system prompt so the LLM generates from context only, (c) tighter streaming timeouts (8s/10s vs 15s/20s), (d) prompt grounding rules reference pre-fetched context instead of tool data. If prefetch is incomplete, fast-path degrades gracefully to standard tool-based flow with normal timeouts. Suggested questions generation starts during streaming (once response reaches 200+ chars) to overlap with remaining stream time; falls back to post-guardrail generation if threshold not reached. Runs in parallel with advisor handoff checks. Intent classification has an in-memory cache (50 entries, 5min TTL) — repeat queries resolve in ~1ms instead of ~2s. All prefetched portfolio data (snapshot, holdings, news, quotes) is injected into enrichmentContext for all prefetch-eligible intents.
- **LLM Resilience & Fallback Provider**: Streaming timeout+retry with AbortController (fast-path: 8s/10s, standard: 15s/20s). Lane 2 → Lane 1 automatic downgrade when both reasoning-model attempts fail — retries with ada-fast at lower max_tokens (4096) for degraded-but-functional responses. All queries go through LLM (no deterministic bypass). `resilientCompletion()` and `resilientStreamCompletion()` helpers in `openaiClient.ts` provide configurable timeout+retry for non-streaming calls. When OpenAI primary provider fails all retries, automatic fallback to Anthropic Claude (claude-sonnet-4-6 via Replit AI Integrations). Anthropic adapter converts OpenAI message format to Anthropic messages API and response back to OpenAI format. Fallback chain: ada-fast → ada-fallback, ada-reason → ada-fallback.
...
- **Verbose/Thinking Mode**: Users can toggle "Think" mode in the chat header to see Ada's reasoning pipeline in real-time. When enabled, `thinking` SSE events stream pipeline steps (PII scan, intent classification, policy evaluation, routing, model selection, data prefetch, LLM generation, guardrails). The `verbose` flag is sent in the `ChatMessageRequest` body and persisted to localStorage. Tenant-level `verbose_mode` feature flag (default: true) gates verbose output; early thinking events are buffered until tenant config is loaded.
  ...
  - **Server-side delivery**: `setImmediate()` async ticks in orchestrator (after early buffer, before data_prefetch) + typed `flush()` in api.ts after thinking events ensure separate SSE chunk delivery.
