...
- **Provider Pattern**: Employs a provider pattern with 7 data provider interfaces (Portfolio, Market, News, Macro, FX, Research, Identity). Phase 1 external providers are connected: Finnhub (market quotes, company news), FRED (macro indicators), SEC EDGAR (filings search), OpenFIGI (instrument identity resolution), Frankfurter (ECB FX rates), CBUAE (AED-specific FX). Each real provider falls back to DB-backed mock implementations via a Proxy-based fallback chain. Provider selection auto-detects based on API key availability. In-memory cache layer with per-data-type TTLs. Rate limiting (per-second for SEC EDGAR, per-minute for others) and sliding-window health tracking with automatic fallback on failure threshold. Phase 2/3 stubs exist for Marketaux, ECB, Twelve Data, FMP, CoinGecko, Yahoo Finance. Registry resolves configurations via env vars or `tenant_config.provider_config`.
